...
#define TEST_EINVAL(f)                  \
  do {                                  \
    assert ((f) == -1 && errno == EINVAL);   \
    errno = 0;                      \
  } while (0)

#define TEST_NULL_EINVAL(f)             \
  do {                                  \
    assert ((f) == NULL && errno == EINVAL); \
    errno = 0;                      \
  } while (0)
...
static unsigned long alloc_fail_seq_new_seq_add_seq_set_name(void) {
  ...
  if ((result = seq_set_name(seq, "01", "string01")) == 1)
    visited |= V(1, 2);
  else if (result == -1 && errno == ENOMEM && seq_get_name(seq, "01") == NULL &&
           seq_set_name(seq, "01", "string01") == 1 &&
           strcmp(seq_get_name(seq, "01"), "string01") == 0)
    visited |= V(2, 2);
  else
...
static unsigned long alloc_fail_seq_new_seq_add_seq_set_name_seq_equiv(void) {
  unsigned long visited = 0;
  seq_t *seq;
  int result;

  while ((seq = seq_new()) == NULL) {}
  while ((result = seq_add(seq, "012")) == -1) {}
  while ((result = seq_set_name(seq, "0", "string0")) == -1) {}
  while ((result = seq_set_name(seq, "01", "string01")) == -1) {}
  while ((result = seq_set_name(seq, "012", "string012")) == -1) {}
  // printf("before:  res=%d (errno: %d - %s); %s %s\n", result, errno, strerror(errno), seq_get_name(seq, "01"), seq_get_name(seq, "012"));
  result = seq_equiv(seq, "01", "012");
  // printf("after:   res=%d (errno: %d - %s); %s %s\n", result, errno, strerror(errno), seq_get_name(seq, "01"), seq_get_name(seq, "012"));

  if (result == 1 && strcmp(seq_get_name(seq, "01"), seq_get_name(seq, "012")) == 0)
    visited |= V(1, 0);
  else if (result == -1 && errno == ENOMEM && strcmp(seq_get_name(seq, "01"), "string01") == 0 &&
           strcmp(seq_get_name(seq, "012"), "string012") == 0)
    visited |= V(2, 0);
  else
    return visited |= V(4, 0);
  
  result = seq_equiv(seq, "012", "0");
  if (result == 1 && strcmp(seq_get_name(seq, "0"), seq_get_name(seq, "012")) == 0)
    visited |= V(1, 0);
  else if (result == -1 && errno == ENOMEM && strcmp(seq_get_name(seq, "0"), "string0") == 0 &&
           strcmp(seq_get_name(seq, "012"), "string01string012") == 0)
    visited |= V(2, 0);
  else
    return visited |= V(4, 0);
...
static int memory(void) {
  ...
  uint status = 0;
  if ((status |= memory_test(alloc_fail_seq_new_seq_add)) != PASS)
    printf("Test 1: FAILED\n");
  if((status |= memory_test(alloc_fail_seq_new_seq_add_seq_set_name)) != PASS)
    printf("Test 2: FAILED\n");
  if((status |= memory_test(alloc_fail_seq_new_seq_add_seq_set_name_seq_equiv)) != PASS)
